...
def _chat_json(system: str, user: str) -> str:
    """เรียก LLM (provider ตาม settings) JSON mode 1 ครั้ง คืน raw JSON string."""
...
def detect_metadata(text: str) -> DetectedMeta:
    ...
    try:
        return DetectedMeta.model_validate_json(_chat_json(_DETECT_SYSTEM, text[:6000]))
...
def improvement_gate(prior_recommendations: list[str], old_text: str, new_text: str) -> GateResult:
    ...
    try:
        return GateResult.model_validate_json(_chat_json(_GATE_SYSTEM, user))
...
def evaluate_proposal(proposal_text: str, context: dict | None = None, lang: str = "en") -> EvaluationLLMOutput:
    ...
    for attempt in range(_MAX_RETRIES + 1):
        try:
            ...
            parsed = EvaluationLLMOutput.model_validate_json(raw)
